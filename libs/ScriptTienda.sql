...
CREATE TABLE venta (id_venta INT,id_cliente VARCHAR(9),id_producto INT,cantidad INT,fecha_venta DATE,PRIMARY KEY (id_venta),FOREIGN KEY (id_cliente) REFERENCES cliente(dni),FOREIGN KEY (id_producto) REFERENCES producto(id_producto));
...
CREATE TABLE asiste ( id_cliente VARCHAR(9), id_empregado INT, primary key(id_cliente,id_empregado),FOREIGN KEY (id_cliente) REFERENCES cliente(dni),
